...
export default function HeroSection() {
  return (
    <section className="relative min-h-[100svh] flex items-center justify-center overflow-hidden">
      {/* Background Dynamic Video (Zero Shadcn) */}
      <div className="absolute inset-0 z-0 overflow-hidden pointer-events-none select-none">
        {/*
          Using a stable cinematic aerial rural background.
        */}
        <video
          autoPlay
          loop
        ...
        {/* Layered Overlays for Depth & High-End Readability */}
        <div className="absolute inset-0 bg-gradient-to-b from-[#0F3D35]/95 via-[#0F3D35]/65 to-[#0F3D35]/95" />
        ...
        <div className="mt-28 grid grid-cols-2 md:grid-cols-4 gap-12 max-w-7xl w-full border-t border-white/10 pt-14 opacity-80">
          {[
            ...
            <div key={stat.label} className="group">
              <div className="text-4xl md:text-5xl font-serif font-extrabold text-[#17A98B] mb-2 drop-shadow-sm group-hover:scale-110 transition-transform">{stat.value}</div>
